...
We aim to train the neural network with raw data without preliminary physical knowledge to find the hidden patterns of the data by itself, and finally to construct the physics from it.
...
<center><img src="/assets/images/DMFTNN/flow.png" width="100%" height="100%"></center>
...
### Test: on the Matsubara frequency domain
...
<center><img src="/assets/images/DMFTNN/result_Matsubara.png" width="100%" height="100%"></center>

Fig(a) shows our neural network with one hidden layer works well on the Matsubara frequency domain, as well as on the real frequency domain. Surprisingly, Fig(b) also shows that the logistic regression model works well on the Matsubara frequency domain, having high accuracy of predicting the phase transition points.

## Analyzing the neural network
We claim that there is an universal pattern that can be used in recognizing phase transition. Below figure illustrates (above, 1) the heat maps for the weight matrix $\mathbf{W}^{(1)}(\omega_n)$, (middle, 2) its kernel density estimation $\mathbf{W}^{(1)}(\omega)$, and lastly, (below, 3) transformed version $\mathbf{W}^{\prime(1)}(i\omega_n)$. Each column indicates different models; Neural network with one hidden layer, hidden node number of (a) $N_h = 100$, (b) $N_h=10$. (c) Logistic regression.

<center><img src="/assets/images/DMFTNN/weightmap.png" width="100%" height="100%"></center>

Conspicuous feature that the weight matrix learned is the high weight assignment on the zero frequency region, both on the real frequency and the Matsubara frequency domain.

To look closely, another figure is introduced below; (left) the heat maps for the weight matrix $\mathbf{W}^{(1)}(\omega_n)$, (right) and its kernel density estimation of $j$-th node(row) of the weight matrix $\mathbf{W}^{(1)}(\omega)$. Each row indicates different models; Neural network with one hidden layer, hidden node number of (a) $N_h = 100$, (b) $N_h=10$. (c) Logistic regression.

<center><img src="/assets/images/DMFTNN/fig3_weightmap.png" width="100%" height="100%"></center>

The conclusion is, that the zero frequency region plays a significant role in predicting phase transition, and the following research is about extracting order parameter from the bath orbitals of DMFT-ED.

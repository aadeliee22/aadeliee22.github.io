...
4. Determine whether the proposed moves(=cluster) will be accepted(=flipped), using original Hamiltonian. The acceptance ratio is given by
   $$
   \begin{aligned}
   A_s(a\to b) &= \min\left(1, \frac{p(b)q(a|b)}{p(a)q(b|a)}\right) 
   &= \min\left(1, \frac{p(b)p_{eff}(a)}{p(a)p_{eff}(b)}\right) = \min(1, \exp(-\beta\{(E_b-E_b^{eff})-(E_a-E_a^{eff})\}))
   \end{aligned}
   $$
...
2. Accept this cluster flip regarding given Hamiltonian, which includes 2nd-NN and 3rd-NN.
   $$
   \begin{aligned}
   A^*(a\to b) &= \min\left(1, \frac{p(b)q(a|b)}{p(a)q(b|a)}\right) 
   &= \min\left(1, \frac{p(b)p_{J_1}(a)}{p(a)p_{J_1}(b)}\right) = \min(1, \exp(-\beta[(E_b-E_b^{J_1})-(E_a-E_a^{J_1})]))
   \end{aligned}
   $$
...
<center><img src="/assets/images/ising_pic/IV_2(1).png" width="80%" height="80%"></center>
